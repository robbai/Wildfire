 ...
 * I thank whatisaphone for linking this in his bot's README
 */
...
public class Biarc extends Curve {
	...
	public Biarc(Vector2 start, Vector2 startDir, Vector2 end, Vector2 endDir){
		...
		this.arcOneLength = this.circleOne.getSectorCircumference(this.start.minus(this.circleOne.getCentre()).angle(this.connection.minus(this.circleOne.getCentre())));
		this.arcTwoLength = this.circleTwo.getSectorCircumference(this.end.minus(this.circleTwo.getCentre()).angle(this.connection.minus(this.circleTwo.getCentre())));
	}
	...
	private double chooseD(){
		...
		// Note: I didn't actually deal with case 3, since it's so rare to occur in these conditions
	...
	private Pair<Circle, Circle> findCircles(){
		...
		double cOneRadius = this.connection.minus(this.start).dotProduct(this.connection.minus(this.start)) / (nOne.scaled(2).dotProduct(this.connection.minus(this.start)));
		Vector2 cOneCentre = this.start.plus(nOne.scaled(cOneRadius));		
		
		Vector2 nTwo = this.endDir.cross();
		double cTwoRadius = this.connection.minus(this.end).dotProduct(this.connection.minus(this.end)) / (nTwo.scaled(2).dotProduct(this.connection.minus(this.end)));
	...
	private Vector2 findConnection(){
		...
		Vector2 v = this.end.minus(this.start);
		return this.start.plus(this.startDir.scaled(dOne)).plus(this.endDir.scaled(v.dotProduct(this.endDir) - this.startDir.scaled(this.dOne).dotProduct(this.endDir)));
	}
	...
	public void render(WRenderer renderer, boolean verbose){
		...
		if(this.dTwo.isPresent()){
			...
			renderer.drawLine3d(Color.WHITE, this.start.plus(this.startDir.scaled(this.dOne)), this.end.plus(this.endDir.scaled(-this.dTwo.getAsDouble())));
		}
		...
		if(verbose){
			renderer.drawString2d("Radii: " + (int)this.circleOne.getRadius() + "uu, " + (int)this.circleTwo.getRadius() + "uu", Color.WHITE, new Point(0, 20), 2, 2);
