...
//    	this.stateSetting.speed(input.info.impact.getTime() > 0.8 || input.car.hasWheelContact ? 1 : 0.25);
//    	this.renderer.drawString3d((int)input.info.jumpImpactHeight + "uu", Color.WHITE, input.car.position, 3, 3);
			...
			try{
				if(lastQuickChat != -1){
					if(Behaviour.isTeammateCloser(input)){
						RLBotDll.sendQuickChat(this.playerIndex, false,
								(new Random()).nextBoolean() ? QuickChatSelection.PostGame_ThatWasFun
		...
		String printInfo = (effectiveState.getName() + (effectiveState.hasAction()
				? " (" + effectiveState.currentAction.getName() + ")"
				: (effectiveState.runningMechanic() ? " (" + effectiveState.currentMechanic.getName() + ")" : "")));
		if(printStates){
	...
	private ControlsOutput fallback(InfoPacket input){
		...
		if(fallbackState.hasAction() && fallbackState.currentAction.expire(input))
			fallbackState.currentAction = null;
	...
	public ControllerState processInput(GameTickPacket packet){
		if(this.recordPackets){
			while(this.gameTickPackets.size() > GAME_PACKET_HISTORY)
				this.gameTickPackets.removeFirst();
			if(!packet.gameInfo().isKickoffPause() && packet.gameInfo().isRoundActive())
				this.gameTickPackets.add(packet);
		}
...
//        }
		...
		if(currentTime > lastQuickChat + quickChatCooldown && lastQuickChat != -1){
			...
			try{
				RLBotDll.sendQuickChat(this.playerIndex, teamOnly,
						quickChatSelection[random.nextInt(quickChatSelection.length)]);
			}catch(Exception e){
	...
	public GameTickPacket gameTickPacketAgo(double seconds){
		if(this.gameTickPackets.isEmpty())
			return null;

		double currentSecondsElapsed = this.gameTickPackets.get(this.gameTickPackets.size() - 1).gameInfo()
				.secondsElapsed();
