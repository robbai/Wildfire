...
public class Utils {
	...
	public static double distanceToWall(Vector3 position){
		return Math.min(Constants.PITCH_WIDTH - Math.abs(position.x),
				Math.abs(position.y) > Constants.PITCH_LENGTH ? 0 : Constants.PITCH_LENGTH - Math.abs(position.y));
	}

	public static Vector2 traceToX(Vector2 start, Vector2 direction, double x){
		// The direction is pointing away from the X
		if(Math.signum(direction.x) != Math.signum(x - start.x))
			return null;
	...
	public static Vector2 traceToY(Vector2 start, Vector2 direction, double y){
		// The direction is pointing away from the X
		if(Math.signum(direction.y) != Math.signum(y - start.y))
			return null;
	...
	public static Vector2 traceToWall(Vector2 start, Vector2 direction){
		...
		if(!direction.isZero()){
			...
			if(trace != null && Math.abs(trace.y) <= Constants.PITCH_LENGTH)
				return trace;

			trace = traceToY(start, direction, Math.signum(direction.y) * Constants.PITCH_LENGTH);
			if(trace != null && Math.abs(trace.x) <= Constants.PITCH_WIDTH)
				return trace;
		}
	...
	public static double distanceFromLine(Vector2 point, Vector2 lineA, Vector2 lineB){
		return Math.abs(
				point.x * (lineB.y - lineA.y) - point.y * (lineB.x - lineA.x) + lineB.x * lineA.y - lineB.y * lineA.x)
	...
	public static Vector3 toLocalFromRelative(CarOrientation carOrientation, Vector3 relative){
		double localRight = carOrientation.right.x * relative.x + carOrientation.right.y * relative.y
				+ carOrientation.right.z * relative.z;
		double localNose = carOrientation.forward.x * relative.x + carOrientation.forward.y * relative.y
				+ carOrientation.forward.z * relative.z;
		double localRoof = carOrientation.up.x * relative.x + carOrientation.up.y * relative.y
				+ carOrientation.up.z * relative.z;
	...
	public static double pointLineSegmentT(Vector3 P, Pair<Vector3, Vector3> lineSegment){
		return closestPointToLineSegment(P.flatten(),
				new Pair<Vector2, Vector2>(lineSegment.getOne().flatten(), lineSegment.getTwo().flatten())).getTwo();
	}
	...
	public static Vector3 toGlobal(Vector3 position, CarOrientation orientation, Vector3 local){
		return position.plus(orientation.right.scaledToMagnitude(local.x))
				.plus(orientation.forward.scaledToMagnitude(local.y)).plus(orientation.up.scaledToMagnitude(local.z));
	}
