...
public class Behaviour {
	...
	public static boolean isTeammateCloser(DataPacket input, Vector3 target){
		...
		for(byte i = 0; i < input.cars.length; i++){
			...
			if(i == input.playerIndex || car == null || car.isDemolished || car.team != input.car.team)
				continue;
			if(ourDistance > target.distance(car.position))
				return true;
		}
		return false;
	}
	...
	public static boolean hasTeammate(DataPacket input){
		for(byte i = 0; i < input.cars.length; i++){
			if(i != input.playerIndex && input.cars[i].team == input.car.team)
				return true;
		}
		return false;
	}
	...
	public static CarData closestOpponent(DataPacket input, Vector3 target){
		...
		for(CarData c : input.cars){
			if(c == null || c.isDemolished || c.team == input.car.team)
				continue;
	...
	public static boolean hasOpponent(DataPacket input){
		for(byte i = 0; i < input.cars.length; i++){
			if(input.cars[i].team != input.car.team)
				return true;
		}
		return false;
	}
	...
	/**
	 * Returns a 2D vector of a point inside of the enemy's goal, it should be a
	 * good place to shoot relative to this car
	 */
	...
	public static boolean isInCone(CarData car, Vector3 target, double threshold){
		if(car.sign * car.position.y > Constants.PITCH_LENGTH)
			return false; // Inside enemy goal.
		Vector2 trace = Utils.traceToY(car.position.flatten(), target.minus(car.position).flatten(),
	...
	public static boolean isOpponentBehindBall(DataPacket input){
		for(byte i = 0; i < input.cars.length; i++){
			CarData car = input.cars[i];
			if(car == null || car.team == input.car.team)
				continue;
			if(correctSideOfTarget(car, input.ball.position))
				return true;
		}
		return false;
	}
	...
	public static boolean defendNotReturn(DataPacket input, Vector3 impactPoint, double homeZoneSize, boolean onTarget){
		return onTarget || input.car.sign * input.ball.velocity.y < -1100
				|| impactPoint.distanceFlat(Constants.homeGoal(input.car.team)) < homeZoneSize;
	}
...
//		return !car.hasWheelContact && (car.position.z > 150 || Math.abs(car.velocity.z) > 280);
	...
	public static Vector3 getBounce(BallPrediction ballPrediction){
		...
		for(int i = 0; i < ballPrediction.slicesLength(); i++){
			...
			if(location.z <= Constants.BALL_RADIUS + 15)
				return location;
		}
		return null;
	}
	...
	/**
	 * Time from now for the time to touch the floor, measured in seconds
	 */
	...
	public static boolean isOnTarget(BallPrediction ballPrediction, int team){
		...
		for(int i = 0; i < ballPrediction.slicesLength(); i++){
			Vector3 location = new Vector3(ballPrediction.slices(i).physics().location());
			if(Math.abs(location.y) >= Constants.PITCH_LENGTH)
				return Math.signum(location.y) != Utils.teamSign(team);
		}
		return false;
	}
	...
	public static boolean isOnPrediction(BallPrediction ballPrediction, Vector3 slicePosition, int start, int end){
		for(int i = Math.max(0, start); i < Math.min(ballPrediction.slicesLength(), end); i++){
			...
			if(distance < velocity.magnitude() / 60)
				return true;
		}
		return false;
	}
	...
	public static boolean isOnPredictionAroundGlobalTime(BallPrediction ballPrediction, Vector3 slicePosition,
			double globalPathTime, int window){
	...
	public static CarData getGoalkeeper(CarData[] cars, int team, double maxGoalDistance){
		...
		for(CarData car : cars){
			if(car.team != team)
				continue;
			if(car.position.flatten().distance(goal) < maxGoalDistance)
				return car;
		}
	...
	public static boolean nobodyElseIntersect(int ourIndex, CarData[] cars, BallPrediction ballPrediction){
		for(int i = 0; i < ballPrediction.slicesLength(); i++){
			...
			if(Math.abs(ballPosition.y) > Constants.PITCH_LENGTH + Constants.BALL_RADIUS + 10)
				break;
			double time = (ballPrediction.slices(i).gameSeconds() - cars[0].elapsedSeconds);

			for(CarData car : cars){
				if(car.index == ourIndex)
					continue;
				double distance = ballPosition.distance(car.position);
				if(distance < 800 || distance / time < 2300)
					return false;
			}
		}
