...
public class JumpPhysics {
	...
	public static final double tick = (1D / 60);
	public static final double maxPressTime = 0.2;
	public static final double minJumpHeight = getMaxHeight(1D / 120), maxJumpHeight = getMaxHeight(maxPressTime);
	...
	public static Vector3 simCar(CarData car, double press, double time){
		...
		final Vector3 jumpAcc = car.orientation.up.scaled(Constants.JUMP_HOLD_ACCELERATION * step);

		Vector3 position = car.position;
		Vector3 velocity = car.velocity.plus(car.orientation.up.scaled(Constants.JUMP_VELOCITY));
