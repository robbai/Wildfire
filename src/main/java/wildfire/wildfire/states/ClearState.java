...
public class ClearState extends State {
	...
	public boolean ready(InfoPacket input){
		// General conditions.
		if(input.gameInfo.isMatchEnded() || Behaviour.isKickoff(input) || Behaviour.isCarAirborne(input.car)
				|| input.car.sign * input.ball.position.y > 2000 || input.info.impact.getPosition().z > 400){
			return false;
		}

		if(Behaviour.isTowardsOwnGoal(input.car, input.info.impact.getPosition()))
			return false;
		...
		if(!Behaviour.isOpponentBehindBall(input)
				&& input.info.impact.getPosition().distanceFlat(input.car.position) < 3000){
	...
	public ControlsOutput getOutput(InfoPacket input){
		...
		if(input.info.impact.getTime() < 3 && Utils.toLocal(input.car, input.info.impact.getBallPosition()).z > 180
				&& car.onFlatGround){
...
//			Slice candidate = SmartDodgeAction.getCandidateLocation(wildfire.ballPrediction, car, trace);
			...
			if(candidate == null || candidate.getFrame() < input.info.impact.getFrame() - (0.2 * 60))
				candidate = input.info.impact;
			if(candidate != null){
				...
				if((candidate.getPosition().y - car.position.y) * car.sign > -1000
						|| !Behaviour.isOnTarget(wildfire.ballPrediction, car.team)){
		...
		if(car.hasWheelContact){
			...
			boolean travellingToBall = (car.velocity.normalised()
					.dotProduct(impactPosition.minus(car.position).normalised()) > 0.8 && car.forwardVelocityAbs > 700);
//			boolean backflip = (Math.abs(dodgeRadians) > 0.75 * Math.PI);

			if((input.info.impact
					.getTime() < (travellingToBall ? Behaviour.IMPACT_DODGE_TIME : Behaviour.IMPACT_DODGE_TIME - 0.08))
		...
		if(car.hasWheelContact && input.info.impact.getPosition().z > (ballSpeedAtCar > 1700 ? 300 : 500)
				&& input.info.impact.getPosition().y * car.sign < 0 && car.position.z < 140){
			double maxRange = input.info.impact.getPosition().z * 4;
			double minRange = input.info.impact.getPosition().z * 1.3;
			if(Utils.isPointWithinRange(car.position.flatten(), input.info.impact.getPosition().flatten(), minRange,
					maxRange)){
				currentAction = AerialAction.fromBallPrediction(this, car, wildfire.ballPrediction,
						input.info.impact.getPosition().z > 800);
				if(currentAction != null && !currentAction.failed)
					return currentAction.getOutput(input);
				currentAction = null;
			}
		...
		// We are in position for the ball to hit us (and we can't quickly turn towards
		// the ball).
		if(car.onFlatGround && car.position.y * car.sign > -Constants.PITCH_LENGTH + 200
				&& Math.abs(impactRadians) > Math.toRadians(55)){
			...
			Vector2 intersect = Utils.traceToY(input.ball.position.flatten(), input.ball.velocity.flatten(),
					car.position.y);
			if(ballTime < 1.5 && intersect != null){
				// Check if our X-coordinate is close-by when we should intersect with the
				// ball's path.
				double xDifference = Math.abs(car.position.x - intersect.x);
				boolean closeby = (xDifference < 0.45 * Utils.lerp(Constants.RIPPER.y, Constants.RIPPER.x,
						Math.abs(car.orientation.forward.flatten().normalised().x)));
				wildfire.renderer.drawLine3d(closeby ? Color.CYAN : Color.BLUE, input.ball.position.flatten(),
						intersect);
				wildfire.renderer.drawString2d("Stop" + (closeby ? " (" + (int)xDifference + "uu)" : ""), Color.WHITE,
						new Point(0, 20), 2, 2);
				if(closeby){
		...
		double offsetMagnitude = (55
				+ 45 * Math.pow(input.info.impact.getPosition().minus(car.position).normalised().y, 2));
		if(Behaviour.correctSideOfTarget(car, input.info.impact.getPosition().flatten())){
			...
			offset = new Vector2(offsetMagnitude * -Math.signum(Utils.traceToWall(car.position.flatten(),
					input.info.impact.getPosition().minus(car.position).flatten()).x), 0);
		}
	...
	private ControlsOutput drivePoint(CarData car, Vector3 destination, boolean allowBackwards){
		if(Math.abs(car.position.y) > Constants.PITCH_LENGTH)
			destination = destination.withX(Utils.clamp(destination.x, -700, 700));
		...
		if(Handling.insideTurningRadius(car, destination) && !reverse){
			double maxVelocity = Math.min(DrivePhysics.maxVelForTurn(car, destination.withZ(car.position.z)),
		...
		ControlsOutput controls = (reverse ? Handling.steeringBackwards(car, destination)
				: Handling.steering(car, destination));
		return controls.withThrottle(throttle)
				.withBoost(controls.holdBoost() && Math.abs(radians) < (rush ? 0.3 : 0.2));
	}
