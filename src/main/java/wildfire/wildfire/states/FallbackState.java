...
public class FallbackState extends State {
	...
	public ControlsOutput getOutput(InfoPacket input){
		...
		if(!smartDodge){
			...
			wildfire.renderer.renderPrediction(wildfire.ballPrediction, Color.RED, input.info.impact.getFrame(),
					jumpImpact.getFrame());
			wildfire.renderer.drawCrosshair(car, jumpImpact.getBallPosition(), Color.ORANGE, 105);
		}

		// Avoid own-goaling.
		Vector2 trace = Utils.traceToY(car.position.flatten(), impactPosition.minus(car.position).flatten(),
				car.sign * -Constants.PITCH_LENGTH);
		boolean avoidOwnGoal = !Behaviour.correctSideOfTarget(car, input.ball.position) && trace != null;
		if(avoidOwnGoal){
			boolean tight = (Math.abs(impactPosition.minus(car.position).flatten().normalised().x) > 0.7
					&& Math.abs(impactPosition.y) > 3000);
			Vector3 avoidOffset = new Vector3(-Math.signum(trace.x) * Utils.clamp(impactDistance / 4.5, 60, 400),
					tight ? -car.sign * 30 : 0, 0);
			impactPosition = impactPosition.plus(avoidOffset);
			wildfire.renderer.drawCrosshair(car, impactPosition, Color.PINK, 125);
		}

		// Target.
		Vector3 localTarget = getLocalPosition(car, new Vector3(),
				Utils.toLocal(car, goal.withZ(Constants.BALL_RADIUS)), 0, Utils.toLocal(car, impactPosition));
		...
		if(Behaviour.isCarAirborne(car)){
		...
		}else if(input.info.impact.getTime() < Behaviour.IMPACT_DODGE_TIME
				&& velocityTowardsImpact > (wall ? 900 : 1200) && Math.abs(car.forwardVelocity) > 0.95){
			double dodgeAngle = steerImpact;
			double goalAngle = Vector2.angle(input.info.impact.getPosition().minus(car.position).flatten(),
					goal.minus(car.position.flatten()));

			// Check if we can go for a shot.
			Vector2 traceGoal = Utils.traceToY(car.position.flatten(),
					input.info.impact.getPosition().minus(car.position).flatten(), car.sign * Constants.PITCH_LENGTH);
			boolean shotOpportunity = (traceGoal != null && Math.abs(traceGoal.x) < 1300);

			if(Math.abs(input.ball.velocity.z) > 500 || wall || !shotOpportunity ? (Math.abs(dodgeAngle) < 0.3)
					: (goalAngle < 0.5 || car.sign * input.ball.velocity.y < -500
							|| car.sign * car.position.y < -3000)){
		...
		}else if(car.forwardVelocity < -800 && ((input.info.impact.getTime() < Behaviour.IMPACT_DODGE_TIME
				&& Math.abs(input.info.impactRadians) < 0.15)
				|| Behaviour.dodgeDistance(input.car) < input.info.impactDistanceFlat)){
			...
			if(car.boost < 10 || Math.abs(steerImpact) > 0.08 || input.info.impact.getTime() < 2.5
					|| Utils.distanceToWall(car.position) < 300){
		...
		if(insideTurningRadius)
			wildfire.renderer.drawTurningRadius(Color.WHITE, car);

		double maxVelocity = (insideTurningRadius && !movingSlow
				? Math.min(DrivePhysics.maxVelForTurn(car, target), Constants.SUPERSONIC_VELOCITY)
...
//		ControlsOutput controls = Handling.forwardDrive(car, target, false);
		ControlsOutput controls = (car.forwardVelocity > (car.boost > 10 ? -350 : -250)
				? Handling.forwardDrive(car, target, false)
				: Handling.chaosDrive(car, target, true));
		if(controls.getThrottle() < -Constants.COAST_THRESHOLD)
			return controls;
		return controls.withThrottle(throttle)
				.withBoost(controls.holdBoost() && Math.abs(radians) < 0.2 && !car.isSupersonic && throttle > 1);
	}
