...
public class WallHitState extends State {
	...
	public boolean ready(InfoPacket input){
		if(Math.abs(input.car.position.y) > Constants.PITCH_LENGTH + 10)
			return false;

		// The ball must be in an appropriate wall hit position.
		if(!isAppropriateWallHit(input.car, input.info.impact.getPosition()))
			return false;

		// We would like the hit to be constructive
		return input.car.sign * input.info.impact.getPosition().y < -4000 || input.car.sign
				* (input.info.impact.getPosition().y - input.car.position.y) > (Behaviour.hasTeammate(input) ? -900
						: -600);
	}
		...
		if(wall){
			...
			if(!isAppropriateWallHit(car, input.info.impact.getPosition())){
				...
				if(Math.abs(target.y) < Constants.PITCH_LENGTH - 800){
					...
					target = target.plus(
							new Vector3(75 * Math.signum(car.position.x - input.info.impact.getPosition().x), 0, 0));
				}
				...
				if((localTarget.z > 85 || highCar)
						&& localTarget.flatten().magnitude() < (car.velocity.magnitude() > 950 ? 380 : 270)
						&& Math.abs(radians) < Math.toRadians(60)){
//				if((localTarget.z > 85 || highCar) && input.info.impact.getTime() < Behaviour.IMPACT_DODGE_TIME - 0.05 && Math.abs(radians) < Math.toRadians(60)){
					...
					if(!currentAction.failed){
						if(highCar)
							wildfire.sendQuickChat(QuickChatSelection.Reactions_Calculated);
						return currentAction.getOutput(input);
					}
				}
...
//				return new ControlsOutput().withThrottle(1).withBoost(Math.abs(radians) < 0.16).withSteer(radians * -4).withSlide(Math.abs(radians) > 1.2 && Handling.canHandbrake(car));
			...
			boolean sideWall = (Constants.PITCH_WIDTH - Math.abs(input.info.impact.getPosition().x) < maxWallDistance
					+ 50);
			boolean backWall = (Constants.PITCH_LENGTH - Math.abs(input.info.impact.getPosition().y) < maxWallDistance
					+ 50) || !sideWall;
			wildfire.renderer.drawString2d((sideWall ? "Side" : (backWall ? "Back" : "Unknown")) + " Wall", Color.WHITE,
					new Point(0, 40), 2, 2);

			// Define the destination.
			Vector2 destination = Utils.traceToWall(input.car.position.flatten(),
					input.info.impact.getBallPosition().minus(input.car.position).flatten());
			if(sideWall){
			...
			destination = destination.withX(Math.max(Math.abs(input.info.impact.getPosition().x), 1200)
					* Math.signum(input.info.impact.getPosition().x));

			if(car.position.distanceFlat(destination) < 1200){
				if(sideWall)
					destination = destination.withX(destination.x * 1.5);
				if(backWall)
					destination = destination.withY(destination.y * 1.5);
			}
...
//				}
			...
			// Controls.
			if(reverse)
				radians = Utils.invertAim(radians);
			return new ControlsOutput().withThrottle(reverse ? -1 : 1)
					.withBoost(Math.abs(radians) < 0.1 && !car.isSupersonic).withSteer(-radians * 3F)
					.withSlide(!reverse && Math.abs(radians) > Math.PI * 0.6);
		}
	}

	private boolean isAppropriateWallHit(CarData car, Vector3 target){
		if(Utils.distanceToWall(target) > maxWallDistance)
			return false;

		boolean backWall = (target.y * car.sign < -4400);

		if(target.z < Math.max(backWall ? 330 : 400, car.position.distanceFlat(target) / 5))
			return false;
		if(Math.abs(target.y) < 4350)
			return true;
