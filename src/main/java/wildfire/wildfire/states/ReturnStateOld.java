...
public class ReturnStateOld extends ReturnState {
	...
	public boolean ready(InfoPacket input){
		if(Behaviour.isKickoff(input) || Behaviour.isCarAirborne(input.car))
			return false;

		boolean opponentBehind = Behaviour.isOpponentBehindBall(input);

		if(input.info.impact.getTime() < input.info.enemyImpactTime - (opponentBehind ? 0.15 : 0.45))
			return false;
		...
		// Just hit it instead.
		if(input.info.impact.getPosition().distanceFlat(input.car.position) < Math.max(1100,
				input.car.velocity.magnitude() * 0.75)
		...
		if(input.car.position.distanceFlat(homeGoal) < 2800){
			...
			if(Behaviour.isTeammateCloser(input)){
				return input.info.impact.getTime() < (6D
						- DrivePhysics.maxVelocity(input.car.velocity.magnitude(), input.car.boost) / 1400D)
