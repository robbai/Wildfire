...
//				&& this.defenderDistance < input.car.position.distanceFlat(input.ball.position) * (input.car.isSupersonic ? 1.2 : 1.4);
				&& this.defenderDistance < input.car.position.distanceFlat(input.ball.position) * (Behaviour.correctSideOfTarget(this.defender, input.ball.position) ? 1.1 : 0.8);
	}
...
//		return 2400 + (Math.max(this.defenderVelocity / 1.2, 1200) + this.defenderDistance / 1.6) * 1.25;
		return 1500 + (Math.max(this.defenderVelocity / 1.4, 1100) + this.defenderDistance / 1.8) * 1.25;
	}
