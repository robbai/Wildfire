...
//		// We want to hit it!
		if(input.info.impact.getTime() < (correctSide ? 0.35 : 0.2)) return false;
		
		if(input.info.impact.getTime() < input.info.enemyImpactTime - 0.06) return false;
...
//		ControlsOutput controls = Handling.chaosDrive(car, destination, false);
		...
		return controls
				.withThrottle(Utils.clamp(controls.getThrottle(), -throttleCap, throttleCap))
...
//		return 2400 + (Math.max(this.defenderVelocity / 1.2, 1200) + this.defenderDistance / 1.6) * 1.25;
		return 1400 + (Math.max(this.defenderVelocity / 1.4, 1100) + this.defenderDistance / 1.8) * 0.7 + carDistance * 0.3;
	}
	...
	private Pair<Integer, Pair<Double, Double>> getDefender(CarData[] cars, Impact[] impacts){
		...
		for(int i = 0; i < cars.length; i++){
			...
			if(Physics.timeToHitGround(car) > 0.5) continue;
...
//			if((bestDefender == null && velocity > -1100)
			if(bestDefender == -1
					|| (ballDistance < bestDistance || velocity > bestVelocity * 1.5)){
				bestDefender = i;
		...
		return new Pair<Integer, Pair<Double, Double>>(bestDefender, new Pair<Double, Double>(bestDistance, bestVelocity));
	}
