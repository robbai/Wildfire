...
public class ShootState extends State {
	...
	public boolean ready(InfoPacket input){
		...
		if(Behaviour.isKickoff(input) || car.sign * car.position.y > Constants.PITCH_LENGTH)
			return false;

		// Not while in goal.
		if(Behaviour.isOnTarget(wildfire.ballPrediction, car.team) && Math.abs(car.position.y) > 4500)
			return false;

		// Not while the ball is being awkward.
		if(input.info.impact.getPosition().z > 200)
			return false;

		// Not during a weird dribble.
		if(input.ball.position.distanceFlat(car.position) < Constants.BALL_RADIUS && input.ball.position.z > 110){ // &&
																													// input.ball.position.distanceFlat(Utils.enemyGoal(car.team))
																													// >
																													// 6000
			return false;
		}
		...
		if(input.info.impact.getTime() > 3 && !car.isSupersonic)
			return false;

		double aimBall = Handling.aim(car, input.info.impact.getPosition().flatten());
		if(Math.abs(aimBall) > Math.PI * 0.7 && car.velocity.magnitude() > 1100)
			return false;
...
// 			double ballVelocityTowards = input.ball.velocity.normalized().dotProduct(car.position.minus(input.ball.position).normalized());
			...
			}else if(Math.abs(impactRadians) > Math.PI * 0.6 && ballDistance > 500 && car.velocity.magnitude() < 600
					&& input.ball.velocity.magnitude() < 1200){
				currentAction = new HopAction(this, input, input.info.impact.getPosition().flatten());
			}else if((input.info.impact.getTime() < 0.3 && Math.abs(impactRadians) < 0.3)
					|| (input.info.impact.getTime() > 2 && Math.abs(impactRadians) < 0.25
							&& car.velocity.magnitude() > 1800)){
				if(!input.ball.velocity.isZero())
					wildfire.sendQuickChat(QuickChatSelection.Information_IGotIt, QuickChatSelection.Reactions_Whew);

				double forwardVelocity = car.forwardVelocity;
				if(forwardVelocity > 1300)
		...
		wildfire.renderer.drawLine3d(Color.WHITE, car.position.flatten(), Utils.traceToY(car.position.flatten(),
