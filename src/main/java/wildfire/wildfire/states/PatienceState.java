...
public class PatienceState extends State {
	...
	public boolean ready(InfoPacket input){
		...
		if(input.info.impact.getPosition().y * input.car.sign < 1000 || input.car.position.z > 200)
			return false;
		double impactDistance = input.car.position.distanceFlat(input.info.impact.getPosition());
		if(impactDistance > 5000)
			return false;

		// Check if we can already shoot
		if(Behaviour.isInCone(input.car, input.info.impact.getPosition(), coneBorder))
			return false;
		...
		if(closestOpponentDistance < Math.max(2300, impactDistance * 1.4)
				|| input.car.sign * input.ball.velocity.y < -850)
			return false;

		int startFrame = input.info.impact.getFrame();
		for(int i = (startFrame + (int)(delaySeconds * 60)); i < Math.min(wildfire.ballPrediction.slicesLength(),
				startFrame + (maxWaitingSeconds * 60)); i++){
		...
		// Don't look like a fool and wait for a straight shot instead of a quick curved
		// shot.
		if(this.point != null){
