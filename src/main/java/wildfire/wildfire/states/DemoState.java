...
public class DemoState extends State {
	...
	private final static double DT = (12D / 60), predictionSeconds = 4D;
...
//		}
	...
	public ControlsOutput getOutput(InfoPacket input){
		...
		for(int i = 0; i < (predictionSeconds / DT); i++){
			...
			if(i != 0){
				wildfire.renderer.drawLine3d(Color.YELLOW, prediction.get(i - 1).flatten(), position.flatten());
			}

			// Acceleration handling
			// https:// samuelpmish.github.io/notes/RocketLeague/ground_control/#throttle
			double currentVelocity = velocity.magnitude();
			double maxAcceleration = DrivePhysics.determineAcceleration(currentVelocity, 1, true);
			if(acceleration.magnitude() > maxAcceleration){
				acceleration = acceleration.scaledToMagnitude(maxAcceleration);
			}

			// Velocity handling
			velocity = velocity.plus(acceleration.scaled(DT));
			if(velocity.magnitude() > Constants.MAX_CAR_VELOCITY){
