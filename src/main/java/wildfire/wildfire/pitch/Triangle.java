...
public class Triangle implements Comparable<Triangle> {
	...
	public Triangle(ReadableObj obj, ObjFace face){
		...
		for(int v = 0; v < 3; v++) vectors[v] = Pitch.toVector(obj.getVertex(face.getVertexIndex(v)));
	}
	...
	public Vector3 segmentIntersects(Vector3 segment1, Vector3 segment2){
		...
		if(D == 0){
			...
			if(N == 0) return vectorInside(segment1) ? segment1 : null;
		...
		// Compute intersect.
		double sI = N / D;
		if(sI < 0 || sI > 1) return null; // No intersection.
	...
	private boolean vectorInside(Vector3 point){
	     return (sameSide(point, this.vectors[0], 1, 2) 
	    		 && sameSide(point, this.vectors[1], 0, 2) 
