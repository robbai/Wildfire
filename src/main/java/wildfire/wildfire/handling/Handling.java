...
public class Handling {
	...
	public static ControlsOutput driveDownWall(DataPacket input){
		double radians = aim(input.car,
				input.car.position.plus(input.car.orientation.up.scaledToMagnitude(70)).flatten());
		return new ControlsOutput().withThrottle(1)
				.withBoost(input.car.forwardVelocity < 1600 && Math.abs(radians) < Math.toRadians(40)
						&& input.car.position.z > 300)
				.withSteer(-4 * radians)
				.withSlide(input.car.forwardVelocityAbs > 500 && Math.abs(radians) > Math.toRadians(70)
						&& canHandbrake(input.car) && input.car.velocity.normalised().dotProduct(Vector3.Z) < 0.4);
	}

	public static ControlsOutput atba(DataPacket input, Vector3 target){
		return new ControlsOutput().withSteer(-Math.signum(aim(input.car, target.flatten()))).withBoost(false)
				.withSlide(false).withThrottle(1);
	}
	...
	public static ControlsOutput chaosDrive(CarData car, Vector3 destination, boolean rush){
		...
		return controls.withThrottle(reverse ? -1 : 1).withBoost(controls.holdBoost() && distance > (rush ? 0 : 1500))
	...
	public static ControlsOutput forwardDrive(CarData car, Vector3 destination, boolean powerTurn){
		...
		if(powerTurn && car.forwardVelocity < 450){
			...
			if(Math.abs(angle) > Math.toRadians(80)){
				...
				return new ControlsOutput().withBoost(false).withThrottle(-1).withSlide(true)
						.withSteer(-Math.signum(angle));
			}
		}
	...
	public static ControlsOutput stayStill(CarData car){
		return new ControlsOutput().withThrottle(Handling.produceAcceleration(car, car.forwardVelocity * -60))
				.withBoost(false);
	}
...
//		double driveDistance = (fullDistance - finalVelocity * peakTime);
		...
		if(renderer != null){
			...
			renderer.drawString2d(
					"Drive Time: " + Utils.round(driveTime) + "s, Peak Time: " + Utils.round(peakTime) + "s",
					Color.WHITE, new Point(0, 40), 2, 2);
			renderer.drawString2d("Initial Vel.: " + (int)initialVelocity + "uu/s", Color.WHITE, new Point(0, 60), 2,
					2);
			renderer.drawString2d("Final Vel.: " + (int)finalVelocity + "uu/s", Color.WHITE, new Point(0, 80), 2, 2);
			renderer.drawString2d("Acceleration: " + (int)acceleration + "uu/s^2", Color.WHITE, new Point(0, 100), 2,
					2);
		}
		...
		if(fullDistance > 0/* && Behaviour.correctSideOfTarget(car, ballPosition) */){
			...
			if(maxVel > finalVelocity){
				acceleration /= Math.max(1,
						(maxVel - finalVelocity) / Utils.lerp(300, 650, finalVelocity / Constants.MAX_CAR_VELOCITY));
...
//	}
	...
	public static ControlsOutput steering(CarData car, Vector3 destination){
		...
		return controls.withBoost(controls.holdBoost() && car.hasWheelContact && !car.isSupersonic)
	...
	public static ControlsOutput steeringBackwards(CarData car, Vector3 destination){
		...
		ControlsOutput controls = steeringBlind(yawError, yawVelocity);
		return controls.withBoost(false).withSlide(
				controls.holdHandbrake() && car.hasWheelContact && canHandbrake(car) && car.forwardVelocity < 0);
	}
	...
	public static ControlsOutput steeringBlind(double yawError, double yawVelocity){
		...
		return new ControlsOutput().withSteer(steer).withBoost(Math.abs(yawError) < Math.toRadians(15))
