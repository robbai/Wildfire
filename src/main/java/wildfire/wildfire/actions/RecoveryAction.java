...
public class RecoveryAction extends Action {
	...
	private final int fallDepth = (int)(2.6 / fallStep), // It only takes around 2.5 seconds to fall from the ceiling,
	...
	public ControlsOutput getOutput(InfoPacket input){
		boolean canBoostDown = (input.car.orientation.forward.dotProduct(Vector3.Z.scaled(-1)) > zToBoost
				&& input.car.position.z > 150);

		Vector3[] fall = simulateFall(boostDown ? (canBoostDown ? Color.RED : Color.YELLOW) : Color.WHITE,
				input.car.position, input.car.velocity);
		...
		wildfire.renderer.drawString2d("Est. Time: " + Utils.round(intersectTime) + "s", Color.WHITE, new Point(0, 40),
				2, 2);

		// whatisaphone's Secret Recipe.
		boostDown = (input.car.boost >= 1
				&& (intersect == null || (intersectTime > 0.6 && intersectLocation.z < Constants.CEILING - 500)));

		boolean planWavedash = (!input.car.hasDoubleJumped && !boostDown && input.car.velocity.z < -420
				&& input.car.orientation.up.z > 0.8 && (intersect != null && intersectLocation.z < 10));
...
//			desiredRoof = input.car.velocity;
		...
		if(planWavedash){
			...
			if(desiredNose.isZero())
				desiredNose = input.car.velocity.withZ(0);
			desiredNose = desiredNose.normalised().withZ(0.3);
			if(input.car.position.z < 50){
				// Perform the wave-dash.
				return new ControlsOutput().withPitch(-1).withJump(true).withYaw(0).withRoll(0);
			}
		}

		// Render the target orientations.
		wildfire.renderer.drawLine3d(Color.RED, input.car.position,
				input.car.position.plus(desiredNose.scaledToMagnitude(200)));
		wildfire.renderer.drawLine3d(Color.GREEN, input.car.position,
				input.car.position.plus(desiredRoof.scaledToMagnitude(200)));

		return new ControlsOutput().withPitchYawRoll(AirControl.getPitchYawRoll(input.car, desiredNose, desiredRoof))
				.withBoost(canBoostDown).withJump(false).withThrottle(
						Math.abs(input.car.velocityDir(input.car.orientation.up)) < 400 && intersectTime < 0.2 ? 1 : 0); // Throttle
																															// to
																															// avoid
																															// turtling.
	}
	...
	private Vector3[] simulateFall(Color colour, Vector3[] positions, Vector3 velocity, int depth){
		...
		if(velocity.magnitude() > 2300)
			velocity = velocity.scaledToMagnitude(2300);
	...
	private Pair<Triangle, Slice> getIntersect(Vector3[] fall){
		for(int i = 0; i < fall.length; i += fallStepIndex){
			...
			// Intersected.
			if(intersect != null){
				return new Pair<Triangle, Slice>(intersect.getOne(), new Slice(intersect.getTwo(),
						Utils.lerp(i, j, Utils.pointLineSegmentT(intersect.getTwo(), lineSegment)) * fallStep));
			}
		}
		return null;
	}
...
//		wildfire.renderer.drawFlatSquare(Color.GREEN, intersect.getTwo(), triangle.getVector(1).minus(triangle.getVector(2)), triangle.getNormal(), 500);
	...
	// double Z = Math.min(Math.abs(Math.abs(carPosition.z) - Constants.PITCHWIDTH),
	// Math.abs(carPosition.z));
	//
