...
public class DodgeGrabber extends Grabber {
	...
	public static final Vector3 origin = new Vector3(0, 0, Constants.CARHEIGHT), noseVector = new Vector3(0, 1, 0), rightVector = new Vector3(1, 0, 0), roofVector = new Vector3(0, 0, 1);
	...
	public DodgeGrabber(Grabby grabby){
		super(grabby, "dodge.csv", new String[] {"input_angle", "velocity_angle", "displace_angle", "impulse_angle", "displace_forward", "displace_side", "start_speed", "end_speed", "time_taken"});
		this.dodge = null;
	...
	public ControlsOutput processInput(DataPacket input){
		if(saved) return new ControlsOutput().withNone();
		...
		if(this.speedIndex >= 0 && this.speedIndex < speeds.length && this.angleIndex >= 0 && this.angleIndex < angles.length){
			renderer.drawString2d("Speed: " + (int)speeds[this.speedIndex], Color.WHITE, new Point(0, 30), 2, 2);
			renderer.drawString2d("Angle: " + (int)Math.toDegrees(angles[this.angleIndex]), Color.WHITE, new Point(0, 60), 2, 2);
			renderer.drawString2d((int)((double)(this.angleIndex * speeds.length + this.speedIndex + 1) / (speeds.length * angles.length) * 100D) + "%", Color.WHITE, new Point(0, 90), 2, 2);
			renderer.drawString2d(dodge == null ? "null" : (Utils.round(dodge.timeDifference(input.elapsedSeconds)) + "s"), Color.CYAN, new Point(0, 120), 2, 2);
		}
		...
		if(input.car.hasWheelContact && this.dodge == null){
			if(input.elapsedSeconds - this.timeSet < 0.1){
				...
				this.currentData = new Object[] {angles[this.angleIndex], null, null, null, null, null, input.car.forwardVelocity, null, null};
...
//		if(dodge != null) return this.dodge.getOutput(input);
		double throttle = Handling.produceAcceleration(input.car, (speeds[this.speedIndex] - input.car.forwardVelocity) * 60);
	...
	private void setupState(boolean stop){
		...
		Vector3 ballPosition = new Vector3(3000, 3000, Constants.BALLRADIUS);
		...
		float boost = 0;
		gameState.withCarState(grabby.getIndex(), new CarState().withBoostAmount(boost).withPhysics(new PhysicsState()
