...
public class CarOrientation extends Rotator {
	...
	public CarOrientation(Vector3 noseVector, Vector3 rightVector, Vector3 roofVector){
		super(Math.atan2(noseVector.z, Math.sqrt(rightVector.z * rightVector.z + roofVector.z * roofVector.z)), // Pitch.
	...
	public CarOrientation(double pitch, double yaw, double roll){
		this(new Vector3(-1 * Math.cos(pitch) * Math.cos(yaw), Math.cos(pitch) * Math.sin(yaw), Math.sin(pitch)),
				new Vector3(Math.cos(roll) * Math.sin(pitch) * Math.cos(yaw) + Math.sin(roll) * Math.sin(yaw),
						Math.cos(yaw) * Math.sin(roll) - Math.cos(roll) * Math.sin(pitch) * Math.sin(yaw),
	...
	public CarOrientation step(double dt, Rotator angularVelocity){
		return new CarOrientation(this.pitch + angularVelocity.pitch * dt, this.yaw + angularVelocity.yaw * dt,
				this.roll + angularVelocity.roll * dt);
	}
